...
| NVIM_DAP_HYDRA_ENABLED                    | Enable Hydra transient keymap menu for DAP debugging commands.               | N/A          | string | Set to "1" to enable. When enabled, `<Leader>d` opens a Hydra menu with single-letter shortcuts. When disabled, standard `<Leader>d*` keymaps are used.              |
| NVIM_DAP_LEGACY_LAUNCH_JS_LOADING_ENABLED | Use legacy manual loading of .vscode/launch.json before DAP continue.        | N/A          | string | Set to "1" to enable. Newer nvim-dap versions auto-load launch.json. Use this flag if you experience regressions and need the old explicit loading behavior.         |
